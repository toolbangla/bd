...
import { getPremiumProduct, type PremiumPackage } from "@/lib/premium-pricing";
...
type GlobalPricing = { single_use_price: number | null; five_use_price: number | null };
...
export default function PremiumToolAccess({ toolId }: PremiumToolAccessProps) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!user.user) return;
      const pricingResponse = await fetch("/api/premium/pricing", { headers: { Authorization: `Bearer ${(await supabase.auth.getSession()).data.session?.access_token ?? ""}` } });
      if (pricingResponse.ok) setPricing(await pricingResponse.json() as GlobalPricing);
